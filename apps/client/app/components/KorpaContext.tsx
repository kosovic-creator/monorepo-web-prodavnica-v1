  ...
  const fetchKorpa = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const normalizedStavke = stavkeData.map((stavka: any) => ({
          ...stavka,
          id: String(stavka.id),
          proizvod: stavka.proizvod
            ? {
                ...stavka.proizvod,
                id: String(stavka.proizvod.id),
              }
            : stavka.proizvod,
        }));
        setStavke(normalizedStavke);

        // Calculate total items count
        const totalItems = normalizedStavke.reduce((acc, stavka) => acc + stavka.kolicina, 0);
